...
bool PLYReader::readMesh(const string &filename, TriangleMesh &mesh)
{
	...
	loadVertices_binary(fin, nVertices, plyVertices);
	loadFaces_binary(fin, nFaces, plyTriangles);
	fin.close();
...
void PLYReader::rescaleModel(vector<float> &plyVertices)
{
	...
	float largestSize = glm::max(size[1][0] - size[0][0], glm::max(size[1][1] - size[0][1], size[1][2] - size[0][2])) * 7.0f;
